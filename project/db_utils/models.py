...
class EnglishTest(db.Model):

    __tablename__ = 'english_test'
    __table_args__ = {'extend_existing': True}
    question_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, nullable=True)
    question = db.Column(db.String(250), nullable=True)
    ...
    def __init__(self, user_id, question, create_date=None, update_time=None, correct_answer=None):
        # format the timestamp and cast the string into a datetime object
        current_date = datetime.now().strftime('%Y-%m-%d')
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
class EnglishAnswer(db.Model):

    __tablename__ = 'english_answer'
    __table_args__ = {'extend_existing': True}
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    question_id = db.Column(db.Integer, nullable=True)
    user_id = db.Column(db.Integer, nullable=True)
